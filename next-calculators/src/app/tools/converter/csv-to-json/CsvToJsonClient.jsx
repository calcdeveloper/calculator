...
import { ArrowRightLeft, Copy, CheckCircle2, AlertCircle, FileJson, FileSpreadsheet, RefreshCw } from 'lucide-react';
      ...
      Papa.parse(input, {
        ...
        complete: (results) => {
          if (results.errors && results.errors.length > 0) {
            setError(results.errors[0].message || 'Error parsing CSV');
            setOutput('');
          } else {
    ...
    <div className="bg-conv-white rounded-3xl shadow-2xl border border-conv-gray overflow-hidden p-6 md:p-8">
      ...
      <div className="bg-conv-gray/5 border-b border-conv-gray/20 p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <FileSpreadsheet className="w-5 h-5 text-conv-primary" />
          <h2 className="font-bold text-conv-dark">CSV to JSON Converter</h2>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-conv-gray/20">
        ...
        <div className="flex flex-col h-[500px]">
          <div className="bg-conv-gray/5 px-4 py-2 border-b border-conv-gray/20 flex items-center justify-between">
            <span className="text-xs font-bold text-conv-gray uppercase tracking-wider">Input: CSV</span>
          </div>
          <textarea
            value={input}
