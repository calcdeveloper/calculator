...
import { Upload, Download, RefreshCw, ChevronRight, AlertCircle, FileText, CheckCircle, ShieldCheck, FileOutput, FileCode } from 'lucide-react';
import WordToPdfSeo from '@/components/tools/WordToPdfSeo';
...
export default function WordToPdfClient() {
  ...
  const validateAndProcess = async (fileObj) => {
    ...
    if (fileObj.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || fileObj.name.toLowerCase().endsWith('.docx')) {
      setFile(fileObj);
      setError(null);
      ...
      try {
        ...
        console.error(err);
        setError("Failed to load DOCX. It might be corrupted or encrypted.");
        setFile(null);
      }
  ...
  const handleCompile = async () => {
    ...
    try {
      ...
      if (!text || text.trim() === '') {
        throw new Error("No readable text found in this document. It may consist entirely of images or complex unsupported objects.");
      }
      ...
      // Split into paragraphs (Mammoth outputs \n\n for paragraphs usually, or just \n)
      ...
      for (let i = 0; i < paragraphs.length; i++) {
        ...
        if (p === '') {
          // Add a little space for empty paragraphs
          cursorY += lineHeight * 0.5;
          continue;
        }

        // Split long paragraphs into wrapped lines
      ...
      <header className="bg-gradient-to-r from-pdf-primary to-pdf-primaryDark pt-16 pb-32 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <nav className="flex justify-center space-x-2 text-pdf-white/70 text-sm mb-6" aria-label="Breadcrumb">
            ...
            <ChevronRight size={14} className="mt-0.5" /> 
            <span className="text-pdf-white font-medium">Word to PDF</span>
          </nav>
          <h1 className="text-3xl md:text-5xl font-extrabold text-pdf-white mb-6 tracking-tight">
            Word to PDF Compiler
          </h1>
          <p className="text-xl text-pdf-white/90 max-w-2xl mx-auto leading-relaxed">
            Extract text from Word (.docx) documents and compile them into clean PDF files. Zero-trust private processing.
          </p>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 -mt-20 pb-20">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            {!file ? (
              ...
              >
                ...
                <h2 className="text-2xl font-bold mb-2 text-pdf-dark text-center">Click or Drag DOCX Here</h2>
                  ...
                  {!convertedUrl ? (
                    <div className="bg-pdf-white border border-pdf-gray p-6 rounded-2xl shadow-sm h-full flex flex-col justify-center">
                      ...
                      <p className="text-pdf-gray text-center mb-8">
                        This tool bypasses complex proprietary rendering logic to rip pure text data straight out of the Word structure and paint it directly into a clean, unstyled PDF document.
                      </p>
                      ...
                      <div className="bg-blue-50 p-4 border border-blue-100 rounded-xl mb-8 flex gap-3 text-blue-800 text-sm font-medium">
                        ...
                        <p>Privacy First: Because this runs entirely in your browser without MS Office servers, complex tables, graphics, and custom fonts will NOT be preserved. Only readable text is compiled.</p>
                      </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-20 mb-20">
          ...
          <div className="bg-pdf-white p-8 rounded-3xl border border-pdf-gray shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-pdf-gray leading-relaxed">We extract the raw text from your Word document and compile it into a clean, unformatted PDF ready for secure archival.</p>
          </div>
          <div className="bg-pdf-white p-8 rounded-3xl border border-pdf-gray shadow-sm hover:shadow-md transition-shadow">
            <div className="w-12 h-12 bg-pdf-primary/10 text-pdf-primary rounded-xl flex items-center justify-center mb-6">
              <RefreshCw size={24} />
        ...
        {/* SEO Content Section */}
        <WordToPdfSeo />
        <RelatedImagePdfTools currentToolPath="/tools/pdf-tools/word-to-pdf" />
      </main>
