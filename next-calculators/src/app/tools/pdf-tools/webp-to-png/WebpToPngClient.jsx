...
import { Upload, Download, RefreshCw, ChevronRight, AlertCircle, Image as ImageIcon, CheckCircle, Target, ShieldCheck } from 'lucide-react';
import WebpToPngSeo from '@/components/tools/WebpToPngSeo';
...
export default function WebpToPngClient() {
  ...
  const validateAndProcess = (fileObj) => {
    ...
    if (fileObj.type === 'image/webp' || fileObj.name.toLowerCase().endsWith('.webp')) {
      setFile(fileObj);
      setError(null);
      ...
      setError("Please select a valid WebP image file.");
    }
  };
  ...
  const handleConvert = () => {
    ...
    img.onerror = () => {
      setError("Failed to read the WebP image data. The file might be corrupted.");
      setIsProcessing(false);
    };
    img.src = imageUrl;
  };
      ...
      <header className="bg-gradient-to-r from-pdf-primary to-pdf-primaryDark pt-16 pb-32 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <nav className="flex justify-center space-x-2 text-pdf-white/70 text-sm mb-6" aria-label="Breadcrumb">
            ...
            <ChevronRight size={14} className="mt-0.5" /> 
            <span className="text-pdf-white font-medium">WebP to PNG</span>
          </nav>
          ...
          <p className="text-xl text-pdf-white/90 max-w-2xl mx-auto leading-relaxed">
            Instantly extract WebP images into universally compatible, lossless PNGs. 100% private processing.
          </p>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 -mt-20 pb-20">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            {!file ? (
              ...
              >
                ...
                <h2 className="text-2xl font-bold mb-2 text-pdf-dark text-center">Click or Drag WebP Image Here</h2>
                ...
                <p className="text-sm text-pdf-gray/60 mt-6 font-medium text-center">Supports .WEBP formats</p>
              </div>
            ) : (
              // Editor State
              <>
                <div className="lg:w-1/2 flex flex-col bg-pdf-bg/30 rounded-2xl p-6 border border-pdf-gray relative">
                  ...
                  <div className="mt-4 flex justify-between text-sm text-pdf-gray bg-pdf-white p-3 rounded-xl border border-pdf-gray shadow-sm">
                    <span className="flex items-center gap-1">
                      ...
                      <span className="bg-pdf-gray/20 text-pdf-dark px-2 py-0.5 rounded font-bold">WebP</span> 
                ...
                <div className="lg:w-1/2 flex flex-col gap-6">
                  {!convertedUrl ? (
                    <div className="bg-pdf-white border border-pdf-gray p-6 rounded-2xl shadow-sm h-full flex flex-col justify-center">
                      ...
                      <p className="text-pdf-gray text-center mb-8">This conversion is 100% lossless. Any transparent backgrounds in your WebP file will be perfectly preserved in the final PNG.</p>
                      ...
                      <div className="bg-amber-50 p-4 border border-amber-100 rounded-xl mb-8 flex gap-3 text-amber-800 text-sm font-medium">
                        ...
                        <p>Because PNG is an uncompressed lossless format, the resulting file size may be significantly larger than the highly-compressed WebP original.</p>
                      </div>
                    ...
                    <div className="bg-emerald-50 border border-emerald-100 p-8 rounded-2xl shadow-sm flex flex-col items-center text-center justify-center h-full">
                      ...
                      <p className="text-emerald-700/80 mb-8">
                        The pixel data has been successfully mapped into a universally compatible PNG.
                      </p>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-20 mb-20">
          <div className="bg-pdf-white p-8 rounded-3xl border border-pdf-gray shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-pdf-gray leading-relaxed">Processing happens strictly inside your active browser session. Graphics and assets are never uploaded to a cloud server.</p>
          </div>
          <div className="bg-pdf-white p-8 rounded-3xl border border-pdf-gray shadow-sm hover:shadow-md transition-shadow">
            <div className="w-12 h-12 bg-pdf-primary/10 text-pdf-primary rounded-xl flex items-center justify-center mb-6">
              <RefreshCw size={24} />
            ...
            <p className="text-pdf-gray leading-relaxed">Skip the upload and download wait times. Our Web API taps into your local hardware to perform complex pixel mapping instantly.</p>
          </div>
          <div className="bg-pdf-white p-8 rounded-3xl border border-pdf-gray shadow-sm hover:shadow-md transition-shadow">
            <div className="w-12 h-12 bg-pdf-primary/10 text-pdf-primary rounded-xl flex items-center justify-center mb-6">
              <Target size={24} />
            ...
            <p className="text-pdf-gray leading-relaxed">PNG is mathematically lossless. Every single pixel from your WebP image is preserved perfectly, alongside any transparent backgrounds.</p>
          </div>
        </section>

        {/* SEO Content Section */}
        <WebpToPngSeo />
        <RelatedImagePdfTools currentToolPath="/tools/pdf-tools/webp-to-png" />
      </main>
