...
import UnicodeDecoderSeo from "@/components/tools/UnicodeDecoderSeo";
import RelatedEncoderDecoderTools from "@/components/tools/RelatedEncoderDecoderTools";

export default function UnicodeDecoderClient() {
  ...
  const handleDecode = () => {
    if (!input) return;
    try {
      let decoded = input.replace(/\\u([\d\w]{4})/gi, (match, grp) => {
        return String.fromCharCode(parseInt(grp, 16));
      });
      setOutput(decoded);
    } catch (error) {
      setOutput("Error decoding Unicode sequence.");
    }
  };
      ...
      <section className="bg-gradient-to-r from-enc-primary to-enc-primaryDark pt-16 pb-32 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <nav className="flex justify-center space-x-2 text-enc-white/70 text-sm mb-6">
            <span>Tools</span> <ChevronRight size={14} /> <span>Encoder/Decoder</span> <ChevronRight size={14} /> <span className="text-white font-medium">Unicode Decoder</span>
          </nav>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 font-bold text-enc-dark">
                  <FileText size={20} className="text-enc-primary" /> Unicode Escaped Input
                </label>
              ...
              <textarea
                ...
                placeholder="Paste \uXXXX formatted unicode sequences here..."
            ...
            <div className="space-y-4">
              ...
              <textarea
                readOnly
                value={output}
                placeholder="Readable text and emojis will perfectly render here..."
