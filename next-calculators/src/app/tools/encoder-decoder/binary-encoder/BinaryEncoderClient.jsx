...
import BinaryEncoderSeo from "@/components/tools/BinaryEncoderSeo";
import RelatedEncoderDecoderTools from "@/components/tools/RelatedEncoderDecoderTools";

export default function BinaryEncoderClient() {
  ...
  const handleEncode = () => {
    ...
    try {
      ...
      let binary = Array.from(view)
        .map((b) => b.toString(2).padStart(8, "0"))
        .join(addSpaces ? " " : "");
      setOutput(binary);
    } catch (error) {
      setOutput("Error encoding to binary.");
    }
  };
      ...
      <section className="bg-gradient-to-r from-enc-primary to-enc-primaryDark pt-16 pb-32 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <nav className="flex justify-center space-x-2 text-enc-white/70 text-sm mb-6">
            <span>Tools</span> <ChevronRight size={14} /> <span>Encoder/Decoder</span> <ChevronRight size={14} /> <span className="text-white font-medium">Binary Encoder</span>
          </nav>
        ...
        <div className="bg-enc-white rounded-3xl shadow-2xl border border-enc-gray overflow-hidden p-6 md:p-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <label htmlFor="addSpaces" className="font-bold text-enc-dark cursor-pointer select-none">
              Format with spaces (e.g., 01101000 01100101)
            </label>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              ...
              <textarea
                ...
                placeholder="Type raw text, code, or emojis here to convert to raw binary..."
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 font-bold text-enc-dark">
                  <Code size={20} className="text-enc-primary" /> 8-Bit Binary Output
                </label>
                <button 
                  disabled={!output}
              ...
              <textarea
                readOnly
                value={output}
                placeholder="Raw 0s and 1s will instantly appear here..."
                className="w-full h-[350px] p-4 bg-enc-bg/30 border border-enc-gray rounded-2xl font-mono text-sm outline-none transition-all resize-none break-all"
              />
            </div>
          </div>
